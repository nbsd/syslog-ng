...
//----------------------------------------------------------------------
// get_count_entries_from_cr_logger_output
//
// Reads cr_logger encrypted output file to get count of log lines of corresponding plain file
//
// in filename: Full filename of cr_logger encrypted output file
// in entry_size:  Length in bytes for one encrypted entry.
// in the_c: THE_C used in Crash Recovery (typical value: 1.1244)
// out count: Count of log lines in corresponding plain log file
//
// return TRUE in case of success, else FALSE

gboolean get_plain_log_lines_from_cr_logger_enc(const gchar *filename, const gsize entry_size, const gdouble the_c,
                                                gsize *count)
{
  GError *error = NULL;
  gsize num_entries = 0U;
  gsize file_size = 0U;
  gchar *contents = NULL;

  if ((NULL == filename) || (NULL == count))
    {
      g_printerr("ERROR: Invalid input parameter.\n");
      return FALSE; //-- ERROR
    }

  // Check if entry_size is valid
  if (0U == entry_size)
    {
      g_printerr("ERROR: Entry size must be greater than zero.\n");
      return FALSE; //-- ERROR
    }

  if (1.0 >= the_c)
    {
      g_printerr("ERROR: the_c  must be greater than 1.0.\n");
      return FALSE; //-- ERROR
    }

  // Read the entire file into memory
  if (!g_file_get_contents(filename, &contents, &file_size, &error))
    {
  ...
  // Check if file size is a multiple of entry_size
  if ((file_size % entry_size) != 0U)
    {
      g_warning("Warning: File size is not a multiple of entry size. Last entry may be incomplete.\n");
    }

  // Calculate the number of entries
  num_entries = file_size / entry_size;

  //-- get from read value count the count of log lines in the plain log file
  gdouble dn = (gdouble)num_entries / the_c;
  *count = (gsize) dn;
...
//----------------------------------------------------------------------
// Helper to remove all trailing line-feeds from a GString

void trim_all_newlines(GString *s)
{
  if ((!s) || (0U == s->len)) return;

  /* If the string ends with CRLF, remove both bytes first */
  if ((s->len >= 2U) && (s->str[s->len - 2U] == '\r') &&
      (s->str[s->len - 1U] == '\n'))
    {
      g_string_truncate(s, s->len - 2U);
    }

  /* Now strip every remaining trailing LF (even if there are many) */
  while ((s->len > 0U) && (s->str[s->len - 1U] == '\n'))
    {
      g_string_truncate(s, s->len - 1U);
    }
}
