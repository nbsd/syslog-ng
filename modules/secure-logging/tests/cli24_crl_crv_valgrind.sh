...
#   Valgrind is available on the test machine
...
REPORT_LOGGER="${VALGRIND_DIR}/valgrind_cr_logger_${NOW}.txt"
REPORT_VERIFIER="${VALGRIND_DIR}/valgrind_cr_verifier_${NOW}.txt"
REPORT_VERIFIER_TAMPERED="${VALGRIND_DIR}/valgrind_cr_verifier_tampered_${NOW}.txt"
...
if ! valgrind "${VALGRIND_OPTS[@]}" --log-file="${REPORT_LOGGER}" \
    "${BIN}/cr_logger" \
    --key "${TEST}/master.key" \
    ...
    echo "Error: cr_logger failed under Valgrind. Check ${REPORT_LOGGER}"
    cnt_error=$((cnt_error + 1))
fi
...
if [[ ! -e "${TEST}/plainlog.txt.enc" ]]; then
    ...
    if ! valgrind "${VALGRIND_OPTS[@]}" --log-file="${REPORT_VERIFIER}" \
        "${BIN}/cr_verifier" \
        --key "${TEST}/master.key" \
        ...
        echo "Error: cr_verifier failed under Valgrind. Check ${REPORT_VERIFIER}"
        cnt_error=$((cnt_error + 1))
    fi
...
if ! valgrind "${VALGRIND_OPTS[@]}" --log-file="${REPORT_VERIFIER_TAMPERED}" \
    "${BIN}/cr_verifier" \
    --key "${TEST}/master.key" \
    ...
    echo "cr_verifier run under Valgrind. Check ${REPORT_VERIFIER_TAMPERED}"
fi
...
show_valgrind_summary "cr_logger" "${REPORT_LOGGER}"
show_valgrind_summary "cr_verifier" "${REPORT_VERIFIER}"
show_valgrind_summary "cr_verifier (with tampered log file)" "${REPORT_VERIFIER_TAMPERED}"

echo " "
echo " "
echo "You might want to call this script like:"
echo "$0 2>&1 | tee ./current.log"
if [[ ${COPY_TO_HOME_BACKUP} == "true" ]]; then
